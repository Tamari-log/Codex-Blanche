...
const THINKING_LEVEL_OPTIONS = ['low', 'medium', 'high'];
...
function normalizeThinkingLevel(value, fallback = 'medium') {
  const normalized = String(value || '').trim().toLowerCase();
  if (THINKING_LEVEL_OPTIONS.includes(normalized)) return normalized;
  return fallback;
}
...
function schedulePersonaTabsRender(){if(personaSearchRenderTimer)clearTimeout(personaSearchRenderTimer);personaSearchRenderTimer=setTimeout(()=>{personaSearchRenderTimer=null;renderPersonaTabs();},PERSONA_SEARCH_RENDER_DEBOUNCE_MS);}
async function startNewSession({ systemPrompt = '', activePersonaId = null, overrides = null, userSignature = '' } = {}){const id=crypto.randomUUID();let resolvedSystemPrompt=systemPrompt;let resolvedActivePersonaId=activePersonaId;let resolvedOverrides=(overrides&&typeof overrides==='object')?{...overrides}:null;if(activePersonaId===null&&resolvedOverrides===null){resolvedOverrides=getNewSessionModelOverrides();}state.settings.systemPrompt=resolvedSystemPrompt;state.ui.activePersonaId=resolvedActivePersonaId;saveSettings();const baseOverrides=(resolvedOverrides&&typeof resolvedOverrides==='object')?resolvedOverrides:{};const normalizedSignature=typeof userSignature==='string'&&userSignature.trim().length?userSignature.trim():(state.settings.userSignature||'Blanche');state.sessions.unshift({id,title:`会話 ${new Date().toLocaleString('ja-JP')}`,messages:[],systemPrompt:resolvedSystemPrompt,userSignature:normalizedSignature,overrides:createSessionOverrides({...baseOverrides,systemPrompt:resolvedSystemPrompt})});state.activeSessionId=id;await persistState();renderHistory();renderSessionList();renderPersonaTabs();}
...
function openSessionConfigEditor(sessionId){
  ...
  const thinkingLevel=document.createElement('select');
  thinkingLevel.className='border rounded-lg p-2 dark:bg-slate-700 dark:text-white';
  thinkingLevel.innerHTML='<option value="low">シンキング: 低い</option><option value="medium">シンキング: 普通</option><option value="high">シンキング: 高い</option>';
  thinkingLevel.value=normalizeThinkingLevel(current.thinkingLevel ?? state.settings.thinkingLevel);
  ...
  form.append(provider,model,thinkingLevel,signature,searchWrap,tempWrap,maxWrap,systemPrompt);
...
function openPresetConfigEditor(index){
  ...
  thinkingLevel.className='border rounded-lg p-2 dark:bg-slate-700 dark:text-white';
  thinkingLevel.innerHTML='<option value="low">シンキング: 低い</option><option value="medium">シンキング: 普通</option><option value="high">シンキング: 高い</option>';
  thinkingLevel.value=normalizeThinkingLevel(draft.thinkingLevel??state.settings.thinkingLevel);
  ...
  form.append(name,provider,model,thinkingLevel,searchWrap,tempWrap,maxWrap,systemPrompt);
  const actions=document.createElement('div');
  actions.className='flex justify-end gap-2';
  const cancel=document.createElement('button');
  cancel.className='px-3 py-2 rounded bg-slate-200 dark:bg-slate-600 dark:text-white';
...
const panelTools=document.createElement('div');panelTools.className='preset-panel-tools';const historySearchInput=document.createElement('input');historySearchInput.className='preset-search-input';historySearchInput.type='search';historySearchInput.placeholder='チャット履歴を検索';historySearchInput.value=historySearchKeyword;historySearchInput.setAttribute('aria-label','チャット履歴を検索');historySearchInput.oninput=(e)=>{historySearchKeyword=e.target.value.trim().toLowerCase();schedulePersonaTabsRender();};panelTools.appendChild(historySearchInput);w.appendChild(panelTools);
const customRows=customPresets.sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0)).map((p)=>{const row=document.createElement('div');row.className='persona-row';const btn=document.createElement('button');btn.className='persona-tab-btn';if(state.ui.activePersonaId===p.id)btn.classList.add('active');btn.innerText=`${p.pinned?'📌 ':''}${p.name}`;btn.onclick=async()=>{const presetSettings=p.settings||{};await startNewSession({systemPrompt:presetSettings.systemPrompt||'',activePersonaId:p.id,userSignature:typeof presetSettings.userSignature==='string'?presetSettings.userSignature:'',overrides:{provider:presetSettings.provider||null,geminiModel:presetSettings.geminiModel||null,openaiModel:presetSettings.openaiModel||null,allowGeminiSearch:typeof presetSettings.allowGeminiSearch==='boolean'?presetSettings.allowGeminiSearch:null,allowOpenaiSearch:typeof presetSettings.allowOpenaiSearch==='boolean'?presetSettings.allowOpenaiSearch:null,temperature:Number.isFinite(presetSettings.temperature)?Number(presetSettings.temperature):null,maxTokens:Number.isFinite(presetSettings.maxTokens)?Number(presetSettings.maxTokens):null,thinkingLevel:normalizeThinkingLevel(presetSettings.thinkingLevel??state.settings.thinkingLevel)}});closeSystemPresetPanel();};row.appendChild(btn);addMenu(row,[{label:'会話設定を編集',action:async()=>openPresetConfigEditor(p.customIndex)},{label:'名前を編集',action:async()=>{const name=window.prompt('プリセット名を入力',p.name);if(!name?.trim())return;state.personas[p.customIndex].name=name.trim();await persistState();renderPersonaTabs();}},{label:p.pinned?'ピン留め解除':'ピン留め',action:async()=>{state.personas[p.customIndex].pinned=!state.personas[p.customIndex].pinned;await persistState();renderPersonaTabs();}},{label:'削除',action:async()=>deletePersona(p)}]);return row;});
...
function bindSettings(){
  ...
  if(thinkingLevel)thinkingLevel.onchange=()=>{state.settings.thinkingLevel=normalizeThinkingLevel(thinkingLevel.value);saveSettings();};
  ...
  systemPrompt.oninput=()=>{state.settings.systemPrompt=systemPrompt.value;saveSettings({defer:true});};
  systemPrompt.onchange=()=>{state.settings.systemPrompt=systemPrompt.value;saveSettings();};
  userSignature.onchange=()=>{state.settings.userSignature=userSignature.value.trim()||'Blanche';saveSettings();updateVisibleUserSignatures();};
  temperature.oninput=()=>{dom.temperatureValue.innerText=temperature.value;state.settings.temperature=Number(temperature.value);saveSettings({defer:true});};
  maxTokens.oninput=()=>{dom.maxTokensValue.innerText=`${maxTokens.value} / ${maxTokens.max}`;state.settings.maxTokens=Number(maxTokens.value);clampMaxTokensToProviderLimit(state.settings.provider);saveSettings({defer:true});};
  if(renderSpeed){
