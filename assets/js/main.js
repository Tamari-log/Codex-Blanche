...
async function startNewSession({ systemPrompt = '', activePersonaId = null } = {}){const id=crypto.randomUUID();state.settings.systemPrompt=systemPrompt;state.ui.activePersonaId=activePersonaId;saveSettings();state.sessions.unshift({id,title:`会話 ${new Date().toLocaleString('ja-JP')}`,messages:[],systemPrompt});state.activeSessionId=id;await persistState();renderHistory();renderSessionList();renderPersonaTabs();}
...
function getEffectiveSettings(){const session=getActiveSession();const systemPrompt=typeof session?.systemPrompt==='string'?session.systemPrompt:(typeof dom.systemPrompt?.value==='string'?dom.systemPrompt.value:state.settings.systemPrompt);return {...state.settings,systemPrompt};}
