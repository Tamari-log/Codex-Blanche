...
async function startNewSession({ systemPrompt = '', activePersonaId = null, overrides = null } = {}){const id=crypto.randomUUID();state.settings.systemPrompt=systemPrompt;state.ui.activePersonaId=activePersonaId;saveSettings();const baseOverrides=(overrides&&typeof overrides==='object')?overrides:{};state.sessions.unshift({id,title:`会話 ${new Date().toLocaleString('ja-JP')}`,messages:[],systemPrompt,overrides:createSessionOverrides({...baseOverrides,systemPrompt})});state.activeSessionId=id;await persistState();renderHistory();renderSessionList();renderPersonaTabs();}
