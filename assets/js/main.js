...
${BACKGROUND_WARNING_TEXT}`,'ai');let streamedReply='';let hasStreamStarted=false;const shouldStream=state.settings.provider==='gemini';console.info('[stream][send] request start',{provider:state.settings.provider,shouldStream,messageCount:s.messages.length});const onChunk=(delta,fullText)=>{if(!hasStreamStarted){console.info('[stream][send] first chunk',{provider:state.settings.provider,deltaLength:(delta||'').length});}hasStreamStarted=true;streamedReply=fullText||`${streamedReply}${delta||''}`;loading.div.innerText=streamedReply;updateScrollToBottomButtonVisibility();chatArea.scrollTop=chatArea.scrollHeight;};try{const reply=await appApi.generateAssistantReply({ provider: state.settings.provider, messages: [...s.messages], apiKey, settings: getEffectiveSettings(), signal: controller.signal, onChunk: shouldStream?onChunk:undefined });const finalReply=reply||streamedReply;console.info('[stream][send] request end',{provider:state.settings.provider,hasStreamStarted,replyLength:(reply||'').length,streamedReplyLength:streamedReply.length,finalReplySource:reply?'reply':'streamedReply'});if(!hasStreamStarted){await appUi.revealWithQuillEffect(chatArea, loading.div, finalReply);}s.messages.push({role:'ai',text:finalReply});await persistState();renderHistory();}catch(e){console.error('[stream][send] request failed',{provider:state.settings.provider,errorName:e?.name,errorMessage:e?.message,streamedReplyLength:streamedReply.length});if(e?.name==='AbortError'){loading.div.innerText=streamedReply||'生成を中断しました。';}else{const detail=`
