...
function saveSettings(){Object.entries({[STORAGE_KEYS.provider]:state.settings.provider,[STORAGE_KEYS.geminiModel]:state.settings.geminiModel,[STORAGE_KEYS.openaiModel]:state.settings.openaiModel,[STORAGE_KEYS.googleClientId]:state.settings.googleClientId,[STORAGE_KEYS.driveFolderName]:state.settings.driveFolderName,[STORAGE_KEYS.driveFileName]:state.settings.driveFileName,[STORAGE_KEYS.systemPrompt]:state.settings.systemPrompt,[STORAGE_KEYS.userSignature]:state.settings.userSignature,[STORAGE_KEYS.temperature]:state.settings.temperature,[STORAGE_KEYS.maxTokens]:state.settings.maxTokens,[STORAGE_KEYS.renderSpeed]:state.settings.renderSpeed,[STORAGE_KEYS.newSessionProvider]:state.settings.newSessionProvider,[STORAGE_KEYS.newSessionGeminiModel]:state.settings.newSessionGeminiModel,[STORAGE_KEYS.newSessionOpenaiModel]:state.settings.newSessionOpenaiModel,[STORAGE_KEYS.rememberApiKeys]:state.settings.rememberApiKeys,[STORAGE_KEYS.rememberGoogleLogin]:state.settings.rememberGoogleLogin}).forEach(([k,v])=>localStorage.setItem(k,v));sessionStorage.setItem(STORAGE_KEYS.geminiKey,state.settings.geminiKey);sessionStorage.setItem(STORAGE_KEYS.openaiKey,state.settings.openaiKey);if(state.settings.rememberApiKeys){localStorage.setItem(STORAGE_KEYS.geminiKey,state.settings.geminiKey);localStorage.setItem(STORAGE_KEYS.openaiKey,state.settings.openaiKey);}else{localStorage.removeItem(STORAGE_KEYS.geminiKey);localStorage.removeItem(STORAGE_KEYS.openaiKey);}}
function applySettingsToUI(){syncContextSliderLimit();dom.provider.value=state.settings.provider;renderModelOptions();dom.geminiKey.value=state.settings.geminiKey;dom.openaiKey.value=state.settings.openaiKey;dom.rememberApiKeys.checked=state.settings.rememberApiKeys;dom.rememberGoogleLogin.checked=state.settings.rememberGoogleLogin;dom.googleClientId.value=state.settings.googleClientId;dom.driveFolderName.value=state.settings.driveFolderName;dom.driveFileName.value=state.settings.driveFileName;dom.systemPrompt.value=state.settings.systemPrompt;dom.userSignature.value=state.settings.userSignature;dom.temperature.value=state.settings.temperature;dom.temperatureValue.innerText=state.settings.temperature;dom.maxTokens.value=state.settings.maxTokens;dom.maxTokensValue.innerText=`${state.settings.maxTokens} / ${dom.maxTokens.max}`;if(dom.renderSpeed)dom.renderSpeed.value=state.settings.renderSpeed||'normal';if(dom.newSessionProvider){dom.newSessionProvider.value=state.settings.newSessionProvider||state.settings.provider;}renderNewSessionModelOptions();}
...
function getEffectiveSettings(){syncGlobalAiSettingsFromUI();const session=getActiveSession();const overrides=getSessionOverrides(session);const provider=overrides.provider||state.settings.provider;const systemPrompt=typeof overrides.systemPrompt==='string'?overrides.systemPrompt:(typeof session?.systemPrompt==='string'?session.systemPrompt:state.settings.systemPrompt);const geminiModel=overrides.geminiModel||state.settings.geminiModel;const openaiModel=overrides.openaiModel||state.settings.openaiModel;const temperature=Number.isFinite(overrides.temperature)?overrides.temperature:state.settings.temperature;const maxTokens=Number.isFinite(overrides.maxTokens)?overrides.maxTokens:state.settings.maxTokens;const userSignature=session?.userSignature||state.settings.userSignature;const renderSpeed=state.settings.renderSpeed||'normal';return {...state.settings,provider,geminiModel,openaiModel,temperature,maxTokens,systemPrompt,userSignature,renderSpeed};}
function bindSettings(){const {provider,model,geminiKey,openaiKey,rememberApiKeys,rememberGoogleLogin,googleClientId,driveFolderName,driveFileName,systemPrompt,userSignature,temperature,maxTokens,renderSpeed,newSessionProvider,newSessionModel,clearSystemPromptBtn,systemPresetToggle,googleLoginBtn,googleLogoutBtn}=dom;provider.onchange=()=>{state.settings.provider=provider.value||state.settings.provider;syncContextSliderLimit();renderModelOptions();syncGlobalAiSettingsFromUI();};model.onchange=()=>{syncGlobalAiSettingsFromUI();};geminiKey.onchange=()=>{state.settings.geminiKey=geminiKey.value.trim();saveSettings();};openaiKey.onchange=()=>{state.settings.openaiKey=openaiKey.value.trim();saveSettings();};rememberApiKeys.onchange=()=>{state.settings.rememberApiKeys=rememberApiKeys.checked;saveSettings();};rememberGoogleLogin.onchange=()=>{state.settings.rememberGoogleLogin=rememberGoogleLogin.checked;saveSettings();};googleClientId.onchange=()=>{state.settings.googleClientId=googleClientId.value.trim();driveSync.tokenClient=null;saveSettings();};driveFolderName.onchange=()=>{state.settings.driveFolderName=driveFolderName.value.trim()||DEFAULT_DRIVE_FOLDER_NAME;driveSync.folderId=null;driveSync.fileId=null;saveSettings();};driveFileName.onchange=()=>{state.settings.driveFileName=driveFileName.value.trim()||DEFAULT_DRIVE_FILE_NAME;driveSync.fileId=null;saveSettings();};systemPrompt.oninput=()=>{state.settings.systemPrompt=systemPrompt.value;saveSettings();};systemPrompt.onchange=()=>{state.settings.systemPrompt=systemPrompt.value;saveSettings();};userSignature.onchange=()=>{state.settings.userSignature=userSignature.value.trim()||'Blanche';saveSettings();renderHistory();};temperature.oninput=()=>{dom.temperatureValue.innerText=temperature.value;state.settings.temperature=Number(temperature.value);saveSettings();};maxTokens.oninput=()=>{dom.maxTokensValue.innerText=`${maxTokens.value} / ${maxTokens.max}`;state.settings.maxTokens=Number(maxTokens.value);clampMaxTokensToProviderLimit(state.settings.provider);saveSettings();};if(renderSpeed){renderSpeed.onchange=()=>{state.settings.renderSpeed=renderSpeed.value||'normal';saveSettings();};}if(newSessionProvider&&newSessionModel){newSessionProvider.onchange=()=>{const selectedProvider=newSessionProvider.value||'gemini';state.settings.newSessionProvider=selectedProvider;renderNewSessionModelOptions();const selectedModel=newSessionModel.value||'';if(selectedProvider==='gemini')state.settings.newSessionGeminiModel=selectedModel;else state.settings.newSessionOpenaiModel=selectedModel;saveSettings();};newSessionModel.onchange=()=>{if((newSessionProvider.value||'gemini')==='gemini')state.settings.newSessionGeminiModel=newSessionModel.value||state.settings.newSessionGeminiModel;else state.settings.newSessionOpenaiModel=newSessionModel.value||state.settings.newSessionOpenaiModel;saveSettings();};}clearSystemPromptBtn.onclick=()=>{systemPrompt.value='';state.settings.systemPrompt='';saveSettings();};systemPresetToggle.onclick=()=>{state.ui.showSystemPresetPanel=!state.ui.showSystemPresetPanel;renderSystemPresetPanel();};googleLoginBtn.onclick=async()=>{try{await driveSync.signIn(true);await driveSync.pull();}catch(e){driveSync.setStatus(`Drive接続失敗: ${getErrorMessage(e)}`);}};googleLogoutBtn.onclick=async()=>{try{state.settings.rememberGoogleLogin=false;saveSettings();if(dom.rememberGoogleLogin)dom.rememberGoogleLogin.checked=false;await driveSync.signOut();}catch(e){driveSync.setStatus(`Drive接続解除失敗: ${getErrorMessage(e)}`);}};}
...
async function regenerateAt(index){const s=getActiveSession();if(!s?.messages[index])return;const target=s.messages[index];if(target.role!=='user'&&target.role!=='ai')return;const effectiveSettings=getEffectiveSettings();const provider=effectiveSettings.provider||state.settings.provider;const apiKey=provider==='gemini'?state.settings.geminiKey:state.settings.openaiKey;if(!apiKey)return;const context=target.role==='user'?s.messages.slice(0,index+1):s.messages.slice(0,index);s.messages=context;await persistState();renderHistory();const loading=addBubble('思索中...','ai');let streamedReply='';let inkRevealer=null;shouldAutoScrollDuringGeneration=true;try{const apiMessages = buildApiMessages(context);const renderSpeed=effectiveSettings.renderSpeed||'normal';if(loading?.div){inkRevealer=appUi.createInkRevealer({chatArea,el:loading.div,mode:renderSpeed,canAutoScroll:()=>shouldAutoScrollDuringGeneration});}const onChunk=(delta,fullText)=>{const prevStreamed=streamedReply;streamedReply=fullText||`${streamedReply}${delta||''}`;if(inkRevealer){const appendDelta=(delta&&delta.length)?delta:(typeof fullText==='string'&&fullText.startsWith(prevStreamed)?fullText.slice(prevStreamed.length):'');if(appendDelta)inkRevealer.enqueue(appendDelta);}else if(loading?.div){loading.div.innerText=streamedReply;if(shouldAutoScrollDuringGeneration)chatArea.scrollTop=chatArea.scrollHeight;}};const reply=await appApi.generateAssistantReply({ provider, messages: apiMessages, apiKey, settings: effectiveSettings, onChunk });const finalReply=normalizeEditableText(reply||streamedReply);if(inkRevealer){inkRevealer.finish(finalReply);await inkRevealer.waitForIdle();}s.messages.push({role:'ai',text:finalReply});await persistState();if(loading?.wrap){loading.wrap.remove();addBubble(finalReply,'ai',s.messages.length-1,true);}else{renderHistory(true);}}catch(e){if(inkRevealer)inkRevealer.cancel();loading.div.innerText=`エラー：${e.message||e}`;appUi.addTransientDeleteButton(loading.wrap);}finally{shouldAutoScrollDuringGeneration=true;}}
