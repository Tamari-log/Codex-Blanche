...
function renderPersonaTabs(){const w=document.getElementById('system-persona-tabs');w.innerHTML='';const presets=[...SYSTEM_PERSONAS.filter((p)=>!state.hiddenSystemPersonaIds.includes(p.id)).map((p)=>({...p,isSystem:true})),...state.personas.map((p,idx)=>({...p,customIndex:idx,isSystem:false,id:`custom-${idx}`}))];const group=document.createElement('details');group.className='preset-sidebar-group';group.open=true;const summary=document.createElement('summary');summary.innerHTML='保存済みプリセット <span class="preset-sidebar-chevron">▾</span>';group.appendChild(summary);const list=document.createElement('div');list.className='preset-sidebar-list';if(presets.length===0){const empty=document.createElement('p');empty.className='text-xs text-slate-600 dark:text-slate-300 px-2 py-1';empty.innerText='保存済みプリセットはありません';list.appendChild(empty);}presets.forEach((p)=>{const btn=document.createElement('button');const g=document.createElement('div');g.className='persona-row';btn.className='persona-tab-btn';if(state.ui.activePersonaId===p.id)btn.classList.add('active');btn.innerText=p.name;btn.onclick=()=>applyPersona(p);const del=document.createElement('button');del.className='persona-delete-btn';del.innerText='×';del.onclick=()=>deletePersona(p);g.appendChild(btn);g.appendChild(del);list.appendChild(g);});group.appendChild(list);w.appendChild(group);}
