...
${BACKGROUND_WARNING_TEXT}`,'ai');let streamedText='';try{const reply=await appApi.generateAssistantReply({ provider: state.settings.provider, messages: [...s.messages], apiKey, settings: getEffectiveSettings(), signal: controller.signal, onDelta:(_,fullText)=>{streamedText=fullText;loading.div.innerText=fullText||'思索中...';scrollChatToBottom();} });const finalReply=reply||streamedText||'応答を取得できませんでした。';s.messages.push({role:'ai',text:finalReply});await persistState();await appUi.revealWithQuillEffect(chatArea, loading.div, finalReply);renderHistory();}catch(e){if(e?.name==='AbortError'){loading.div.innerText='生成を中断しました。';}else{loading.div.innerText=`エラー：${e.message||e}`;}appUi.addTransientDeleteButton(loading.wrap);}finally{currentRequestController=null;releaseWakeLock();appUi.setThinkingMode(dom.sendBtn, false, { default: SEND_BUTTON_DEFAULT_ICON, stop: SEND_BUTTON_STOP_ICON });userInput.focus();}}
...
async function regenerateAt(index){const s=getActiveSession();if(!s?.messages[index])return;const target=s.messages[index];if(target.role!=='user'&&target.role!=='ai')return;const apiKey=state.settings.provider==='gemini'?state.settings.geminiKey:state.settings.openaiKey;if(!apiKey)return;const context=target.role==='user'?s.messages.slice(0,index+1):s.messages.slice(0,index);s.messages=context;await persistState();renderHistory();const loading=addBubble('思索中...','ai');let streamedText='';try{const reply=await appApi.generateAssistantReply({ provider: state.settings.provider, messages: context, apiKey, settings: getEffectiveSettings(), onDelta:(_,fullText)=>{streamedText=fullText;loading.div.innerText=fullText||'思索中...';scrollChatToBottom();} });const finalReply=reply||streamedText||'応答を取得できませんでした。';s.messages.push({role:'ai',text:finalReply});await persistState();await appUi.revealWithQuillEffect(chatArea, loading.div, finalReply);renderHistory();}catch(e){loading.div.innerText=`エラー：${e.message||e}`;appUi.addTransientDeleteButton(loading.wrap);}}
